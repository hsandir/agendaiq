...
-- Add team members (separate statements)
...
-- Create meetings (with all required fields)
...
SELECT 'Q1 Curriculum Standards Review', 'Quarterly review of curriculum standards alignment with state requirements', 
       ...
       s.id, 2, 2, 2, 'team-curr-dev', 'COMPLETED', '2024-03-10 10:00:00'::timestamp
FROM staff s WHERE s.user_id = 10 LIMIT 1;
...
SELECT 'Science Curriculum Update Planning', 'Planning session for integrating new science standards for grades 6-12',
       ...
       s.id, 2, 2, 2, 'team-curr-dev', 'COMPLETED', '2024-04-15 10:00:00'::timestamp
FROM staff s WHERE s.user_id = 10 LIMIT 1;
...
SELECT 'EdTech Platform Evaluation', 'Evaluation of new educational technology platforms for school-wide implementation',
       ...
       s.id, 3, 2, 2, 'team-tech-int', 'COMPLETED', '2024-03-20 10:00:00'::timestamp
FROM staff s WHERE s.user_id = 11 LIMIT 1;
...
SELECT '1:1 Device Rollout Planning', 'Planning for 1:1 device implementation across all grade levels',
       ...
       s.id, 3, 2, 2, 'team-tech-int', 'COMPLETED', '2024-05-05 10:00:00'::timestamp
FROM staff s WHERE s.user_id = 11 LIMIT 1;
...
SELECT 'Mental Health Support Program Launch', 'Planning launch of comprehensive mental health support program for students',
       ...
       s.id, 4, 2, 2, 'team-wellness', 'COMPLETED', '2024-04-01 10:00:00'::timestamp
FROM staff s WHERE s.user_id = 12 LIMIT 1;
...
SELECT m.id, 'Device Distribution Strategy', 'Need systematic approach for distributing devices across grade levels',
       s.id, 'Decision', 'Resolved', 1, '2024-05-05 10:00:00'::timestamp
FROM meeting m, staff s 
WHERE m.title = '1:1 Device Rollout Planning' AND s.user_id = 11 LIMIT 1;
...
SELECT m.id, 'Wellness Program Activities', 'Students need structured wellness activities and stress reduction programs',
       s.id, 'Information_Sharing', 'Ongoing', 2, '2024-04-01 10:00:00'::timestamp
FROM meeting m, staff s 
WHERE m.title = 'Mental Health Support Program Launch' AND s.user_id = 15 LIMIT 1;
...
SELECT 
    ...
    'DOCUMENT', ARRAY['technology', 'platforms', 'comparison'], 
    u.id, s.id, true, 22, 7, 
...
-- Final summary
SELECT 'Teams Created' as item, COUNT(*) as count FROM teams WHERE id LIKE 'team-%'
UNION ALL SELECT 'Team Members' as item, COUNT(*) as count FROM team_members WHERE team_id LIKE 'team-%'  
UNION ALL SELECT 'Meetings Created' as item, COUNT(*) as count FROM meeting WHERE team_id LIKE 'team-%'
UNION ALL SELECT 'Agenda Items' as item, COUNT(*) as count FROM meeting_agenda_items WHERE meeting_id IN (SELECT id FROM meeting WHERE team_id LIKE 'team-%')
UNION ALL SELECT 'Meeting Attendees' as item, COUNT(*) as count FROM meeting_attendee WHERE meeting_id IN (SELECT id FROM meeting WHERE team_id LIKE 'team-%')
UNION ALL SELECT 'Meeting Notes' as item, COUNT(*) as count FROM meeting_notes WHERE meeting_id IN (SELECT id FROM meeting WHERE team_id LIKE 'team-%')
UNION ALL SELECT 'Knowledge Resources' as item, COUNT(*) as count FROM team_knowledge WHERE team_id LIKE 'team-%';

-- Show team details
SELECT 
    t.name,
    t.description,
    COUNT(tm.staff_id) as members,
    COUNT(DISTINCT m.id) as meetings,
    COUNT(DISTINCT tk.id) as knowledge_items
FROM teams t
